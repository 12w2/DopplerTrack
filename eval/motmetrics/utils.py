...
def compare_to_groundtruth(gt, dt,pcd_path, dist='iou', distfields=None, distth=0.5):
    ...
    if distfields is None:
        distfields = ['X', 'Y', 'Width', 'Height','Visibility']

    def compute_iou(a, b,frame):
        return iou_matrix(a, b,pcd_path, frame,max_iou=distth)
    ...
    for fid in allframeids:
        ...
        if len(oids) > 0 and len(hids) > 0:
            dists = compute_dist(fgt.values, fdt.values,fid)
